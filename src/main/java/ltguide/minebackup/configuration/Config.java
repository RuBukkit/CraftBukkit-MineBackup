...
public class Config extends Configuration {
    ...
    protected void migrate() {
        ...
        if (migrate(5, 9, 5)) {
            ...
            set("default_actions.mysql", false);
            set("default_settings.mysql", "3d");

            map.clear();
            map.put("hostname", "localhost");
            map.put("port", 3306);
            map.put("database", "myDatabase");
            map.put("user", "admin");
            map.put("password", "");

            set("mysql", createSection("mysql", map));
        }
    ...
    private void checkMapFormat(final File dir) {
        ...
        if (files.size() > 0)
            plugin.warning(String.format("%% detected old map format files (%s); once backed up, remove %s", dir.getPath(), plugin.joinString(files.toArray(), " and ")));
    }
