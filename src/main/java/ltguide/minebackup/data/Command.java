...
public class Command {
    ...
    public void sendInfo(final CommandSender sender, final String label) {
        if (hasPermission(sender) && !"".equals(getDescription()))
            plugin.send(sender, plugin.getMessage("SYNTAX", getSyntax(label), getDescription()));
    }
    ...
    public Command init(final CommandSender sender, final String label, final String[] list) throws CommandException {
        ...
        if (getRequiredArgs() > args.size())
            throw new CommandException(plugin.getMessage("SYNTAX", getSyntax(label), getDescription()));
