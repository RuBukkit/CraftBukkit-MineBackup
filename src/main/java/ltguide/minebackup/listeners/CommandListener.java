...
public class CommandListener implements CommandExecutor {
    ...
    private void sendStatus(final CommandSender sender, final String type, final String name) {
        ...
        for (final String action : plugin.actions)
            if (plugin.hasAction(action) && (interval = plugin.config.getInterval(type, name, action)) != 0)
                sb.append(plugin.getMessage("STATUS_ACTION", action, getNext(msecs, type, name, action), plugin.convertMilli2Time(interval)));
